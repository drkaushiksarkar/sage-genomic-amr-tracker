"""ForecastingTransformsV4S20: forecasting_transforms_v4_s20 module v4.
...
@dataclass
class ForecastingTransformsV4S20Config:
    """Configuration for forecasting_transforms_v4_s20 v4."""
    batch_size: int = 128
    hidden_dim: int = 512
    num_layers: int = 6
    dropout: float = 0.4
...
class ForecastingTransformsV4S20(nn.Module):
    """Neural network module for forecasting_transforms_v4_s20 v4.
    ...
    def __init__(self, config: Optional[ForecastingTransformsV4S20Config] = None):
        super().__init__()
        self.config = config or ForecastingTransformsV4S20Config()
        c = self.config
        ...
        logger.info("Initialized ForecastingTransformsV4S20 v4 with %d params",
