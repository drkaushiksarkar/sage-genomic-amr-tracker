"""IndexingConfigV2S12: indexing_config_v2_s12 module v2.
...
@dataclass
class IndexingConfigV2S12Config:
    """Configuration for indexing_config_v2_s12 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class IndexingConfigV2S12(nn.Module):
    """Neural network module for indexing_config_v2_s12 v2.
    ...
    def __init__(self, config: Optional[IndexingConfigV2S12Config] = None):
        super().__init__()
        self.config = config or IndexingConfigV2S12Config()
        c = self.config
        ...
        logger.info("Initialized IndexingConfigV2S12 v2 with %d params",
