"""LoaderV501: loader_v501 module v2.
...
@dataclass
class LoaderV501Config:
    """Configuration for loader_v501 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV501(nn.Module):
    """Neural network module for loader_v501 v2.
    ...
    def __init__(self, config: Optional[LoaderV501Config] = None):
        super().__init__()
        self.config = config or LoaderV501Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV501 v2 with %d params",
