"""PipelineV501: pipeline_v501 module v1.
...
@dataclass
class PipelineV501Config:
    """Configuration for pipeline_v501 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class PipelineV501(nn.Module):
    """Neural network module for pipeline_v501 v1.
    ...
    def __init__(self, config: Optional[PipelineV501Config] = None):
        super().__init__()
        self.config = config or PipelineV501Config()
        c = self.config
        ...
        logger.info("Initialized PipelineV501 v1 with %d params",
